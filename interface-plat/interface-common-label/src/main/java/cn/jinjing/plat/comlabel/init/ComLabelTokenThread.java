package cn.jinjing.plat.comlabel.init;

import cn.jinjing.plat.comlabel.util.CacheToken;
...
public class ComLabelTokenThread implements Runnable{

    public static Log log = LogFactory.getLog(ComLabelTokenThread.class);

    //更换成自己的apikey
    private static final String APIKEY = ConfigUtil.getProperties("com_request_apikey");
    //更换成自己的秘钥
    private static final String SECRETKEY = ConfigUtil.getProperties("com_request_pwd");
    //访问的IP地址或域名
    private static final String HOST = ConfigUtil.getProperties("com_request_host");
    //端口
    private static final int PORT = Integer.parseInt(ConfigUtil.getProperties("com_request_port"));
    // 协议
    ...
    public ComLabelTokenThread(long expireTime){
    ...
    public void run() {
        try {
            String token_time = CacheToken.getTokenAndTime();
            ...
            if(update){
                ...
                if(tokenId != null && !"".equals(tokenId)){
                    ...
                    CacheToken.putTokenAndTime(tokenId);
                }else{
    ...
    public static String getReqUrl(String product, String module, String method) {
        String tokenId = CacheToken.getToken();
