...
public class EnterpriseController {
	...
	@ResponseBody
	public String getEnterprise(@PathVariable("getEnterprise") String label,String userCode, String accessKey,String enterprise,String encrypt) {
		...
		try {
			...
			enterpriseName = URLDecoder.decode(enterprise,"utf-8");
			...
			if(Result.SUCCESS.getValue().equals(result.getString("flag"))) {
				...
				if(dataSource != null && "06".equals(dataSource) ) {
					
					//企业搜索
					ro = enterpriseSearchService.getEnterprise(enterpriseName);
					if(ro.isFlag() == false) {
						result.put("flag", Result.ERROR.getValue());
						result.put("message", ro.getMessage());
					}else {//调取数据接口成功
			...
			map.put("address", enterpriseName);
